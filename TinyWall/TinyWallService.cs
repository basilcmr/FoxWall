                ...
                case FirewallMode.JellyMode:
                    {
                        ...
                        // 1. Block all network traffic by default
                        var def = new RuleDef(ModeId, "Block everything", GlobalSubject.Instance, RuleAction.Block, RuleDirection.InOut, Protocol.Any, (ulong)FilterWeights.DefaultBlock);
                        rules.Add(def);

                        // 2. Allow Jellyfin inbound on Local Subnet (TCP/UDP)
                        var jellyfinSubj = new ExecutableSubject(@"C:\Program Files\Jellyfin\Server\jellyfin.exe");
                        var defJellyIn = new RuleDef(ModeId, "Allow Jellyfin Inbound", jellyfinSubj, RuleAction.Allow, RuleDirection.In, Protocol.Any, (ulong)FilterWeights.UserPermit)
                        {
                            RemoteAddresses = RuleDef.LOCALSUBNET_ID
                        };
                        rules.Add(defJellyIn);

                        // 3. Allow Jellyfin outbound on Local Subnet (TCP/UDP)
                        var defJellyOut = new RuleDef(ModeId, "Allow Jellyfin Outbound", jellyfinSubj, RuleAction.Allow, RuleDirection.Out, Protocol.Any, (ulong)FilterWeights.UserPermit)
                        {
                            RemoteAddresses = RuleDef.LOCALSUBNET_ID
                        };
                        rules.Add(defJellyOut);
